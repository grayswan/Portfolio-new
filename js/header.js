(function() {
    ...
    function Line(angle) {
        ...
        this.draw = function() {
            ...
            var r1 = Math.random()*(size < 900 ? 900 : size)*0.9;
            var r2 = Math.random()*(size < 900 ? 900 : size)*0.9;
            ...
            ctx.moveTo(x1, y1);
            ctx.lineTo(x2, y2);
            ctx.strokeStyle = 'rgba(64,64,65,'+(0.5+Math.random()*0.5)+')';

            ctx.stroke();

            ctx.beginPath();
            ctx.arc(x1, y1, 2, 0, 2 * Math.PI, false);
            ctx.fillStyle = 'rgba(51,51,52,'+(0.5+Math.random()*0.5)+')';
            ctx.fill();
